...
export const arange = (n, shift) => Array.from({ length: n }, (_x, i) => i + shift)

export const transpone = a => a[0].map((_, colIndex) => a.map(row => row[colIndex]));
...
export const zeros = (rows, cols) => Array(rows).fill().map(() => Array(cols).fill(0));
