...
export const expr = (x, y, nnn) => {
  ...
  const dX = dotPow(subtract(matrix(X), dotMultiply(cos(dotMultiply(matrix(I), 2 * Math.PI * (1 / nnn))), 2)), 2)
  const dY = dotPow(subtract(matrix(Y), dotMultiply(sin(dotMultiply(matrix(I), 2 * Math.PI * (1 / nnn))), 2)), 2)
